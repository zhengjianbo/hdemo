...
import com.ram.kettle.compatibility.Row;
...
public class RowMeta implements RowMetaInterface {
	...
	public RowMeta clone() {
		...
		int rowMetaSize=size();
		for (int i = 0; i <rowMetaSize ; i++) {
	...
	public void addValueMeta(String name,int type) {
		ValueMetaInterface v = new ValueMeta(name,type);
		v.setOrigin(name);
		addValueMeta(v);
	}
	 ...
	 */
	public String getString(Object[] dataRow, int index) throws KettleException {
		if (dataRow == null) {
			return null;
		}
	 ...
	 *             in case there was a problem converting the data.
	 */
	public Long getInteger(Object[] dataRow, int index) throws KettleException {
		if (dataRow == null) {
			return null;
		}
		ValueMetaInterface meta = valueMetaList.get(index);
	 ...
	 *             in case there was a problem converting the data.
	 */
	public Double getNumber(Object[] dataRow, int index) throws KettleException {
		if (dataRow == null) {
			return null;
		}
		ValueMetaInterface meta = valueMetaList.get(index);
	 ...
	 *             in case there was a problem converting the data.
	 */
	public Date getDate(Object[] dataRow, int index) throws KettleException {
		if (dataRow == null) {
			return null;
		}
		ValueMetaInterface meta = valueMetaList.get(index);
	 ...
	 *             in case there was a problem converting the data.
	 */
	public byte[] getBinary(Object[] dataRow, int index) throws KettleException {
		if (dataRow == null) {
			return null;
		}
		ValueMetaInterface meta = valueMetaList.get(index);
	 ...
	 *             lazy conversion)
	 */
	public boolean isNull(Object[] dataRow, int index) throws KettleException {
		if (dataRow == null) {
			// I guess so...
			return true;
		}
	 ...
	 */
	public Object[] cloneRow(Object[] objects) throws KettleException {
	...
	public String getString(Object[] dataRow, String valueName,
			String defaultValue) throws KettleException {
	...
	/**
	 * Merge the values of row r to this Row. The values that are not yet in the
	 * row are added unchanged. The values that are in the row are renamed to
	 * name_2, name_3, etc.
	 * 
	 * @param r
	 ...
	 */
	public void writeData(DataOutputStream outputStream, Object[] data)
			throws KettleException {
		...
		// to detect an EOF on the other end (sockets etc)
		//
		if (size() == 0) {
			try {
				outputStream.writeBoolean(true);
			} catch (IOException e) {
				throw new KettleException("Error writing marker flag", e);
			}
		}
	}
	...
	public void writeMeta(DataOutputStream outputStream) throws KettleException {
		...
		try {
			...
			throw new KettleException("Unable to write nr of metadata values",
					e);
		}
	...
	public RowMeta(DataInputStream inputStream) throws KettleException,
		...
		try {
			...
			throw new KettleException("Unable to read nr of metadata values: "
					+ e.toString(), e);
		}
		for (int i = 0; i < nr; i++) {
	...
	public Object[] readData(DataInputStream inputStream)
			throws KettleException, KettleException, SocketTimeoutException {
	...
	public void removeValueMeta(String valueName) throws KettleException {
		int index = indexOfValue(valueName);
		if (index < 0)
			throw new KettleException(
					"Unable to find value metadata with name '" + valueName
	...
	/**
	 * @return a string with a description of all the metadata values of the
	 *         complete row of metadata
	 */
	 ...
	 */
	public String getString(Object[] row) throws KettleException {
		StringBuffer buffer = new StringBuffer();
		for (int i = 0; i < size(); i++) {
			if (i > 0)
				buffer.append(", ");
	...
	public int compare(Object[] rowData1, Object[] rowData2, int fieldnrs1[],
			int fieldnrs2[]) throws KettleException {
	 ...
	 * result in a lot of collisions for similar types of data (e.g. [A,B] ==
	 * [B,A] and is not suitable for normal use. It is kept to provide backward
	 ...
	 * @deprecated
	 */
	public int oldXORHashCode(Object[] rowData) throws KettleException {
		int hash = 0;
	...
	/**
	 * Calculates a simple hashCode of all the native data objects in the
	 * supplied row. This method will return a better distribution of values for
	 * rows of numbers or rows with the same values in different positions.
	 ...
	 * hashCodes will be different resulting in the two rows not being
	 * considered equal via the hashCode even though compare() or equals() might
	 * consider them to be.
	 * 
	 * @param rowData
	 *            The data to calculate a hashCode with
	 ...
	 *             in case there is a data conversion error
	 */
	public int hashCode(Object[] rowData) throws KettleException {
		return Arrays.hashCode(rowData);
	}

	/**
	 * Calculates a hashcode of the converted value of all objects in the
	 * supplied row. This method returns distinct values for nulls of different
	 * data types and will return the same hashCode for different native values
	 * that have a ValueMeta converting them into the same value (e.g.
	 ...
	 *             in case there is a data conversion error
	 */
	public int convertedValuesHashCode(Object[] rowData) throws KettleException {
		if (rowData == null)
			return 0;
	 ...
	 */
	public static final byte[] extractData(RowMetaInterface metadata,
			Object[] row) {
		try {
	...
	public static final Object[] getRow(RowMetaInterface metadata, byte[] data) {
		try {
			...
			throw new RuntimeException(
					"Error de-serializing row of data from byte array", e);
		}
	}

	public static Row createOriginalRow(RowMetaInterface rowMeta,
	...
	public static RowMetaInterface clone(RowMetaInterface rowMeta) {
		return rowMeta.clone();
	}
