...
public class ValueMeta implements ValueMetaInterface {
	...
	public static final String[] SINGLE_BYTE_ENCODINGS = new String[] {
			"ISO8859_1", "Cp1252", "ASCII", "Cp037", "Cp273", "Cp277", "Cp278",
			"Cp280", "Cp284", "Cp285", "Cp297", "Cp420", "Cp424", "Cp437",
			"Cp500", "Cp737", "Cp775", "Cp850", "Cp852", "Cp855", "Cp856",
			"Cp857", "Cp858", "Cp860", "Cp861", "Cp862", "Cp863", "Cp865",
			"Cp866", "Cp869", "Cp870", "Cp871", "Cp875", "Cp918", "Cp921",
			"Cp922", "Cp1140", "Cp1141", "Cp1142", "Cp1143", "Cp1144",
			"Cp1145", "Cp1146", "Cp1147", "Cp1148", "Cp1149", "Cp1250",
			"Cp1251", "Cp1253", "Cp1254", "Cp1255", "Cp1257", "ISO8859_2",
			"ISO8859_3", "ISO8859_5", "ISO8859_5", "ISO8859_6", "ISO8859_7",
			"ISO8859_8", "ISO8859_9", "ISO8859_13", "ISO8859_15",
			"ISO8859_15_FDIS", "MacCentralEurope", "MacCroatian",
	...
	private synchronized Date convertStringToDate(String string)
			throws KettleException {
		string = Const.trimToType(string, getTrimType()); // see if trimming
															// needs to be
															// performed before
															// conversion
		...
		try {
			...
			throw new KettleException(toString()
					...
					+ "] to a date using format [" + dateFormat + "]", e);
		}
	}
	...
	private synchronized Double convertStringToNumber(String string)
			throws KettleException {
		string = Const.trimToType(string, getTrimType()); // see if trimming
															// needs to be
															// performed before
		...
		try {
			return new Double(getDecimalFormat(false).parse(string)
					.doubleValue());
		} catch (Exception e) {
			throw new KettleException(toString()
	...
	public synchronized SimpleDateFormat getDateFormat() {
		...
		// That way, we can always convert from Date to String and back without
		// a problem, no matter how complex the format was.
	...
	private synchronized String convertIntegerToString(Long integer)
			throws KettleException {
		if (integer == null) {
			if (!outputPaddingEnabled || length < 1) {
				return null;
	...
	private synchronized Long convertStringToInteger(String string)
			throws KettleException {
		string = Const.trimToType(string, getTrimType()); // see if trimming
															// needs to be
															// performed before
	...
	private synchronized BigDecimal convertStringToBigNumber(String string)
			throws KettleException {
		string = Const.trimToType(string, getTrimType()); // see if trimming
															// needs to be
															// performed before
		...
		try {
			return (BigDecimal) getDecimalFormat(bigNumberFormatting).parse(
					string);
		} catch (Exception e) {
			...
			try {
				...
				throw new KettleException(toString()
						+ " : couldn't convert string value '" + string
						+ "' to a big number.", ex);
			}
		}
	}
	...
	private String convertBinaryStringToString(byte[] binary)
		...
		if (binary == null || binary.length == 0) {
			return (EMPTY_STRING_AND_NULL_ARE_DIFFERENT && binary != null) ? ""
					: null;
		}
	...
	public Object convertToNormalStorageType(Object object)
		...
		default:
			throw new KettleException(toStringMeta()
	 ...
	 */
	public Object cloneValueData(Object object) throws KettleException {
		if (object == null)
			return null;

		if (storageType == STORAGE_TYPE_NORMAL) {
			switch (getType()) {
			case ValueMeta.TYPE_NSTRING:
	...
	public String getCompatibleString(Object object) throws KettleException {
		try {
			String string;
			...
			case TYPE_DATE:
				...
				case STORAGE_TYPE_INDEXED:
					string = object == null ? null
							: convertDateToCompatibleString((Date) index[((Integer) object)
									.intValue()]);
					break;
				default:
					throw new KettleException(toString()
	...
	public String getString(Object object) throws KettleException {
		try {
			String string;
			...
			case TYPE_STRING:
				...
				case STORAGE_TYPE_INDEXED:
					string = object == null ? null
							: (String) index[((Integer) object).intValue()];
					break;
				default:
					throw new KettleException(toString()
							+ " : Unknown storage type " + storageType
			...
			case TYPE_DATE:
				...
				case STORAGE_TYPE_INDEXED:
					string = object == null ? null
							: convertDateToString((Date) index[((Integer) object)
									.intValue()]);
					break;
				default:
					throw new KettleException(toString()
			...
			case TYPE_INTEGER:
				...
				case STORAGE_TYPE_INDEXED:
					string = object == null ? null
							: convertIntegerToString((Long) index[((Integer) object)
									.intValue()]);
					break;
				default:
					throw new KettleException(toString()
			...
			case TYPE_SERIALIZABLE:
				...
				case STORAGE_TYPE_BINARY_STRING:
					...
					break;
				case STORAGE_TYPE_INDEXED:
					string = object == null ? null : index[((Integer) object)
							.intValue()].toString();
					break; // just go for the default toString()
				default:
					throw new KettleException(toString()
			...
			default:
				throw new KettleException(toString() + " : Unknown type "
						+ type + " specified.");
			}
	...
	public Double getNumber(Object object) throws KettleException {
		try {
			if (object == null) // NULL
			{
				return null;
			...
			case TYPE_STRING:
				...
				case STORAGE_TYPE_INDEXED:
					return convertStringToNumber((String) index[((Integer) object)
							.intValue()]);
				default:
					throw new KettleException(toString()
							+ " : Unknown storage type " + storageType
			...
			case TYPE_BOOLEAN:
				...
				case STORAGE_TYPE_INDEXED:
					return convertBooleanToNumber((Boolean) index[((Integer) object)
							.intValue()]);
				default:
					throw new KettleException(toString()
							+ " : Unknown storage type " + storageType
	...
	public Long getInteger(Object object) throws KettleException {
		try {
			if (object == null) // NULL
			{
				return null;
			...
			case TYPE_STRING:
				...
				case STORAGE_TYPE_INDEXED:
					return convertStringToInteger((String) index[((Integer) object)
							.intValue()]);
				default:
					throw new KettleException(toString()
							+ " : Unknown storage type " + storageType
			...
			case TYPE_NUMBER:
				...
				case STORAGE_TYPE_INDEXED:
					return new Long(
							Math.round(((Double) index[((Integer) object)
				...
				case STORAGE_TYPE_BINARY_STRING:
					return new Long(
							((Date) convertBinaryStringToNativeType((byte[]) object))
									.getTime());
				case STORAGE_TYPE_INDEXED:
					return convertDateToInteger((Date) index[((Integer) object)
							.intValue()]);
				default:
					throw new KettleException(toString()
							+ " : Unknown storage type " + storageType
			...
			case TYPE_BOOLEAN:
				...
				case STORAGE_TYPE_INDEXED:
					return convertBooleanToInteger((Boolean) index[((Integer) object)
							.intValue()]);
				default:
					throw new KettleException(toString()
							+ " : Unknown storage type " + storageType
			...
			case TYPE_SERIALIZABLE:
				return convertStringToInteger(""+ object);
			default:
				throw new KettleException(toString() + " : Unknown type "
						+ type + " specified.");
			}
	...
	public BigDecimal getBigNumber(Object object) throws KettleException {
		if (object == null) // NULL
		{
			return null;
		}
		...
		case TYPE_STRING:
			...
			case STORAGE_TYPE_INDEXED:
				return convertStringToBigNumber((String) index[((Integer) object)
						.intValue()]);
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
			...
			case STORAGE_TYPE_BINARY_STRING:
				return BigDecimal
						.valueOf(((Long) convertBinaryStringToNativeType((byte[]) object))
								.longValue());
			case STORAGE_TYPE_INDEXED:
				return BigDecimal.valueOf(((Long) index[((Integer) object)
						.intValue()]).longValue());
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
			...
			case STORAGE_TYPE_BINARY_STRING:
				return BigDecimal
						.valueOf(((Double) convertBinaryStringToNativeType((byte[]) object))
								.doubleValue());
			case STORAGE_TYPE_INDEXED:
				return BigDecimal.valueOf(((Double) index[((Integer) object)
						.intValue()]).doubleValue());
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
		...
		case TYPE_DATE:
			...
			case STORAGE_TYPE_INDEXED:
				return convertDateToBigNumber((Date) index[((Integer) object)
						.intValue()]);
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
		...
		case TYPE_BOOLEAN:
			...
			case STORAGE_TYPE_INDEXED:
				return convertBooleanToBigNumber((Boolean) index[((Integer) object)
						.intValue()]);
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
		...
		case TYPE_BINARY:
			throw new KettleException(
					toString()
							+ " : I don't know how to convert binary values to integers.");
		case TYPE_SERIALIZABLE:
			return convertStringToBigNumber(""+ object);
		default:
			throw new KettleException(toString() + " : Unknown type " + type
					+ " specified.");
		}
	}

	public Boolean getBoolean(Object object) throws KettleException {
		if (object == null) // NULL
		{
			return null;
		}
		...
		case TYPE_STRING:
			...
			case STORAGE_TYPE_INDEXED:
				return convertStringToBoolean(trim((String) index[((Integer) object)
						.intValue()]));
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
		...
		case TYPE_INTEGER:
			...
			case STORAGE_TYPE_INDEXED:
				return convertIntegerToBoolean((Long) index[((Integer) object)
						.intValue()]);
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
		...
		case TYPE_NUMBER:
			...
			case STORAGE_TYPE_INDEXED:
				return convertNumberToBoolean((Double) index[((Integer) object)
						.intValue()]);
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
		...
		case TYPE_BIGNUMBER:
			...
			case STORAGE_TYPE_INDEXED:
				return convertBigNumberToBoolean((BigDecimal) index[((Integer) object)
						.intValue()]);
			default:
				throw new KettleException(toString()
						+ " : Unknown storage type " + storageType
...
//							+ " : I don't know how to convert serializable values to booleans.");
	...
	public Date getDate(Object object) throws KettleException {
		if (object == null) // NULL
		{
			return null;
		}
	...
	public byte[] getBinary(Object object) throws KettleException {
		if (object == null) // NULL
		{
			return null;
		}
	...
	public byte[] getBinaryString(Object object) throws KettleException {
	...
	public void writeData(DataOutputStream outputStream, Object object)
			throws KettleException {
		try {
			// Is the value NULL?
			...
			if (object != null) // otherwise there is no point
			{
				...
				case STORAGE_TYPE_BINARY_STRING:
					...
					// In this case, we opt not to convert anything at all for
					// speed.
					// That way, we can save on CPU power.
					// Since the streams can be compressed, volume shouldn't be
					// an issue at all.
					//
	...
	public Object readData(DataInputStream inputStream) throws KettleException {
		try {
			// Is the value NULL?
			...
			case STORAGE_TYPE_NORMAL:
				...
				default:
					throw new KettleException(toString()
							+ " : Unable to de-serialize data of type "
							+ getType());
				}
			...
			case STORAGE_TYPE_INDEXED:
				return readSmallInteger(inputStream); // just an index: 4-bytes
														// should be enough.
			...
			throw new KettleException(toString()
					+ " : Unable to read value data from input stream", e);
		} catch (Exception e) {
			throw new KettleException(e);
		}
	}

	private void writeString(DataOutputStream outputStream, String string)
			throws IOException {
	...
	private void writeBinaryString(DataOutputStream outputStream,
			byte[] binaryString) throws IOException {
		// Write the length and then the bytes
	...
	private String readString(DataInputStream inputStream) throws IOException {
		...
		// ConstLog.message("Read string("+getName()+"), length "+length+": "+string);
		return string;
	}

	private byte[] readBinaryString(DataInputStream inputStream)
			throws IOException {
		// Read the length and then the bytes
		int length = inputStream.readInt();
		if (length < 0) {
	...
	private void writeBigNumber(DataOutputStream outputStream, BigDecimal number)
			throws IOException {
		String string = number.toString();
		writeString(outputStream, string);
	}

	private BigDecimal readBigNumber(DataInputStream inputStream)
			throws IOException {
		String string = readString(inputStream);
		// ConstLog.message("Read big number("+getName()+") ["+string+"]");
		return new BigDecimal(string);
	}

	private void writeDate(DataOutputStream outputStream, Date date)
			throws IOException {
	...
	private Date readDate(DataInputStream inputStream) throws IOException {
		...
		// ConstLog.message("Read Date("+getName()+") ["+new Date(time)+"]");
		return new Date(time);
	}

	private void writeBoolean(DataOutputStream outputStream, Boolean bool)
			throws IOException {
	...
	private Boolean readBoolean(DataInputStream inputStream) throws IOException {
		...
		// ConstLog.message("Read boolean("+getName()+") ["+bool+"]");
		return bool;
	}

	private void writeNumber(DataOutputStream outputStream, Double number)
			throws IOException {
	...
	private Double readNumber(DataInputStream inputStream) throws IOException {
		...
		// ConstLog.message("Read number("+getName()+") ["+d+"]");
		return d;
	}

	private void writeInteger(DataOutputStream outputStream, Long number)
			throws IOException {
	...
	private Long readInteger(DataInputStream inputStream) throws IOException {
		...
		// ConstLog.message("Read integer("+getName()+") ["+l+"]");
		return l;
	}

	private void writeInteger(DataOutputStream outputStream, Integer number)
			throws IOException {
		outputStream.writeInt(number.intValue());
	}

	private Integer readSmallInteger(DataInputStream inputStream)
			throws IOException {
		Integer i = Integer.valueOf(inputStream.readInt());
		// ConstLog.message("Read index integer("+getName()+") ["+i+"]");
		return i;
	}

	private void writeBinary(DataOutputStream outputStream, byte[] binary)
			throws IOException {
	...
	private byte[] readBinary(DataInputStream inputStream) throws IOException {
		...
		// ConstLog.message("Read binary("+getName()+") with size="+size);

		return buffer;
	}

	public void writeMeta(DataOutputStream outputStream) throws KettleException {
		try {
			int type = getType();
			...
			writeString(outputStream,
					dateFormatLocale != null ? dateFormatLocale.toString()
							: null);

		} catch (IOException e) {
			throw new KettleException(toString()
					+ " : Unable to write value metadata to output stream", e);
		}
	}

	public ValueMeta(DataInputStream inputStream) throws KettleException,
			KettleException {
		this();
		...
		try {
			...
			case STORAGE_TYPE_BINARY_STRING: {
				// In case we do have storage metadata defined, we read that
				// back in as well..
				if (inputStream.readBoolean()) {
			...
			throw new KettleException(toString()
					+ " : Unable to read value metadata from input stream", e);
		}
	}
	...
	public ValueMeta(Node node) throws KettleException {
		...
		storageType = getStorageType(XMLHandler
				.getTagValue(node, "storagetype"));
		...
		case STORAGE_TYPE_INDEXED: {
			...
			for (int i = 0; i < index.length; i++) {
				Node valueNode = XMLHandler.getSubNodeByNr(indexNode, "value",
						i);
				...
				if (Const.isEmpty(valueString)) {
					...
					case TYPE_BIGNUMBER:
						...
						break;
					case TYPE_BOOLEAN:
						index[i] = Boolean.valueOf("Y"
								.equalsIgnoreCase(valueString));
						break;
					case TYPE_BINARY:
					...
					default:
						throw new KettleException(
								toString()
										+ " : Unable to de-serialize indexe storage type from XML for data type "
										+ getType());
					}
				}
			}
		...
		caseInsensitive = "Y".equalsIgnoreCase(XMLHandler.getTagValue(node,
				"case_insensitive"));
		sortedDescending = "Y".equalsIgnoreCase(XMLHandler.getTagValue(node,
				"sort_descending"));
		outputPaddingEnabled = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
				node, "output_padding"));
		dateFormatLenient = "Y".equalsIgnoreCase(XMLHandler.getTagValue(node,
				"date_format_lenient"));
		String dateFormatLocaleString = XMLHandler.getTagValue(node,
				"date_format_locale");
	...
	public Object getValue(Node node) throws KettleException {
		...
		case STORAGE_TYPE_NORMAL:
			...
			case TYPE_BINARY:
				return XMLHandler.stringToBinary(XMLHandler.getTagValue(node,
						"binary-value"));
			default:
				throw new KettleException(toString()
						+ " : Unable to de-serialize '" + valueString
		...
		default:
			throw new KettleException(toString() + " : Unknown storage type "
					+ getStorageType());
		}
	...
	public static final String getStorageTypeCode(int storageType) {
		if (storageType >= STORAGE_TYPE_NORMAL
				&& storageType <= STORAGE_TYPE_INDEXED) {
			return storageTypeCodes[storageType];
		}
		return null;
	}
	 ...
	 *             lazy conversion)
	 */
	public boolean isNull(Object data) throws KettleException {
		try {
			Object value = data;

			if (isStorageBinaryString()) {
				if (value == null || !EMPTY_STRING_AND_NULL_ARE_DIFFERENT
						&& ((byte[]) value).length == 0)
			...
			// A value (5 spaces for example) can be null after trim and
			// conversion
			//
			if (value == null)
				return true;
			...
			// If it's a string and the string is empty, it's a null value as
			// well
			//
			if (isString()) {
			...
			throw new RuntimeException("Unable to verify if [" + toString()
					+ "] is null or not because of an error:" + e.toString(), e);
		}
	}
	 ...
	 * 
	 * @return -1 if <i>one</i> is smaller than <i>two</i>, 0 is both byte
	 * arrays are identical and 1 if <i>one</i> is larger than <i>two</i>
	 * private int compareBinaryStrings(byte[] one, byte[] two) {
	 * 
	 * for (int i=0;i<one.length;i++) { if (i>=two.length) return 1; // larger
	 * if (one[i]>two[i]) return 1; // larger if (one[i]<two[i]) return -1; //
	 ...
	 */
	public int compare(Object data1, Object data2) throws KettleException {
	 ...
	 *             In case we get conversion errors
	 */
	public int compare(Object data1, ValueMetaInterface metaX, Object data2)
			throws KettleException {
		if (metaX == null) {
			throw new KettleException(
					toStringMeta()
		...
		ValueMetaInterface meta2 = metaX.clone();
		if (metaX.getType() == TYPE_NSTRING) {
			meta2.setType(TYPE_STRING);
		}
		...
		try {
			...
			if (getType() == meta2.getType()) {
				...
				case STORAGE_TYPE_INDEXED:
					...
					default:
						throw new KettleException(meta2.toStringMeta()
	...
	public Object convertDataFromString(String pol,
			...
			int trim_type) throws KettleException {
		...
		if (Const.isEmpty(pol)) {
			...
			if (!Const.isEmpty(null_value)) {
				if (null_value.length() <= pol.length()) {
					// If the polled value is equal to the spaces right-padded
					// null_value, we have a match
					//
	 ...
	 * @throws KettleException
	 */
	public int hashCode(Object object) throws KettleException {
		int hash = 0;
	 ...
	 */
	public Value createOriginalValue(Object data) throws KettleException {
		...
		if (isNull(data)) {
			...
			default:
				throw new KettleException(toString()
						+ " : We can't convert data type " + getTypeDesc()
	 ...
	 *             case there is a data conversion problem
	 */
	public Object getValueData(Value value) throws KettleException {
		...
		default:
			throw new KettleException(toString()
					...
					+ value.getTypeDesc() + " to a primitive data type");
		}
	}
	...
	/**
	 * @return true if the String encoding used (storage) is single byte
	 *         encoded.
	 */
	 ...
	 */
	public void setNumberOfBinaryStringConversions(
	...
	/**
	 * @return The available trim type codes (NOT localized, use for
	 *         persistence)
	 */
