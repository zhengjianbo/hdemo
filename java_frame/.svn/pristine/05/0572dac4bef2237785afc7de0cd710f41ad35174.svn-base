...
public class Row implements Comparable<Row>, Serializable {
	...
	/**
	 * Creates a new row as a duplicate of the given row. The values are copied
	 * one by one into new values.
	 * 
	 * @param r
	 ...
	 * 
	 * @return true if the value was found and removed, false if the value
	 *         wasn't found.
	 */
	...
	/**
	 * Merge the values of row r to this Row. Merge means: only the values that
	 * are not yet in the row are added (comparing on the value name).
	 * 
	 * @param r
	...
	/**
	 * Merge the data of row r to this Row. That means: All fields in row r that
	 * do exist in this row (same name and same type) and have non-empty values
	 * will have their values written into this row, if the value of that field
	 * is empty in this row.
	 * 
	 * @param r
	 *            The row to be merged with this row
	 */
	 ...
	 * @return the index of the value with the given name, -1 is nothing was
	 *         found.
	 */
	...
	/**
	 * Print the names and the String representations of the values of the
	 * Values in row to stdout.
	 */
	public void print() {
		int i;
		for (i = 0; i < size(); i++) {
			System.out
					.println("Element[" + i + "] = [" + getValue(i).getName() + "] = " + getValue(i).toString()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
	}
	...
	public String toString() {
		...
		for (int i = 0; i < size(); i++) {
			...
			if (value != null) {
				retval.append(value.getName()).append('=')
	 ...
	 *             if an error occurs.
	 */
	public void write(DataOutputStream dos) throws KettleException {
		try {
			writeObj(dos);
		} catch (Exception e) {
			throw new KettleException("Row.ErrorWritingRow", e); //$NON-NLS-1$
		}
	}
	 ...
	 */
	public Row(DataInputStream dis) throws KettleException {
		try {
			readObj(dis);
		} catch (EOFException e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"Row.EndOfFileReached"), e); //$NON-NLS-1$
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"Row.ErrorReadingRowData"), e); //$NON-NLS-1$
		}
	}
	 ...
	 *            The DataInputStream to read from
	 * @param meta
	 *            The description (name, type, length, precision) of the values
	 *            to be read (the same number of values are read)
	 * @throws KettleException
	 *             if the row couldn't be created by reading from the data input
	 *             stream.
	 */
	public Row(DataInputStream dis, Row meta) throws KettleException {
	 ...
	 *             stream.
	 */
	public Row(DataInputStream dis, int size, Row meta) throws KettleException {
		try {
			// get all values in the row
			for (int i = 0; i < size; i++) {
			...
			throw new KettleException(BaseMessages.getString(PKG,
					"Row.EndOfFileReadingRow"), e); //$NON-NLS-1$
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"Row.RowError"), e); //$NON-NLS-1$
		}
	}

	/**
	 * Write a row of Values to a DataOutputStream, without saving the
	 * meta-data.
	 * 
	 * @param dos
	 *            The DataOutputStream to write to
	 * @return true if the row was written successfuly, false if something went
	 *         wrong.
	 */
	 ...
	 * considering an ascending clause.
	 * 
	 * @param r
	 ...
	 * @param ascending
	 *            an entry for each value to compare where true means and normal
	 *            compare, false the reverse.
	 * @return -1 if the row is smaller, 0 if they are equal and 1 if the row is
	 *         larger.
	 */
	 ...
	 *         larger.
	 */
	public int compare(Row r, int fieldnrs1[], int fieldnrs2[],
			boolean ascending[], boolean caseInsensitive[]) {
		int retval = 0;
		int i;
	...
	/**
	 * Compare 2 rows with each other using one value in the rows and also
	 * considering an ascending clause.
	 * 
	 * @param r
	 *            The row to compare with
	...
	/**
	 * Compare 2 complete rows of values with each other. Strings are compared
	 * in a case insensitive way
	 * 
	 * @param r
	 *            the row to compare with
	 * @return -1 if the row is smaller, 0 if both rows are equal, 1 if the row
	 *         is larger.
	 */
	...
	/**
	 * Returns an exact copy of this row. On purpose not the real clone() method
	 * (notice the uppercase 'C') because of (rhino) javascript problems with
	 * it.
	 * 
	 ...
	 * 
	 * @return the logging date of this row. public Date getLogdate() { return
	 * logdate; }
	 */
	 ...
	 * 
	 * @return the logging time for this row. public long getLogtime() { if
	 * (logdate==null) return 0L; return logdate.getTime(); }
	 */

	/**
	 * Checks whether or not the row is empty A row is empty if all the values
	 * in the row are null A row is empty if there are no values in the row.
	 * 
	 * @return true if the row is considered empty, false if the row is not
	 *         empty.
	 */
	...
	/**
	 * Get an array of strings showing the name of the values in the row padded
	 * to a maximum length, followed by the types of the values.
	 * 
	 * @param maxlen
	 *            The length to which the name will be padded.
	 * @return an array of strings: the names and the types of the fieldnames in
	 *         the row.
	 */
	public String[] getFieldNamesAndTypes(int maxlen) {
		...
		for (int i = 0; i < size(); i++) {
			Value v = getValue(i);
			retval[i] = Const.rightPad(v.getName(), maxlen)
					+ "   (" + v.getTypeDesc() + ")"; //$NON-NLS-1$ //$NON-NLS-2$
		}
	...
	/**
	 * Search for a value, if it doesn't occur in the row, return the default
	 * value.
	 * 
	 * @param valuename
	 ...
	 * @param def
	 *            The default value to return
	 * @return The short integer representation of the value found or the
	 *         default
	 */
	...
	public static final void sortRows(List<Row> rows, int fieldNrs[],
			boolean ascDesc[]) {
	...
	public static final byte[] extractData(Row row) {
		try {
			...
			throw new RuntimeException(BaseMessages.getString(PKG,
					"Row.ErrorSerializing") + row, e); //$NON-NLS-1$
		}
	}
	...
	public static final Row getRow(byte[] data, Row metadata) {
		try {
			...
			throw new RuntimeException(BaseMessages.getString(PKG,
					"Row.ErrorDeserializing"), e); //$NON-NLS-1$
		}
	}
