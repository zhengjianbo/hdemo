...
/**
 * This class contains a Value of type String and the length by which it is
 * described.
 * 
 * @author Matt
 * @since 15-10-2004
...
public class ValueString implements ValueInterface, Cloneable {
	...
	public Date getDate() {
		if (string != null) {
			try {
				return ((SimpleDateFormat) LOCAL_SIMPLE_DATE_PARSER.get())
						.parse(string);
	...
	public long getInteger() {
		return Const.toLong(Const.ltrim(string), 0L); // Remove the leading
														// space to make
	...
	public void setDate(Date date) {
		this.string = ((SimpleDateFormat) LOCAL_SIMPLE_DATE_PARSER.get())
				.format(date);
	}
