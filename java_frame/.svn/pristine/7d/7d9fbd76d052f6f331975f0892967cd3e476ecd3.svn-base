...
public class OracleDatabaseMeta extends BaseDatabaseMeta implements
		DatabaseInterface {

	private String pluginId = "ORACLE";
	...
	public String getSQLQueryFields(String tableName) {
		return "SELECT /*+FIRST_ROWS*/ * FROM " + tableName
				+ " WHERE ROWNUM < 1";
	}
	...
	public String getURL(String hostname, String port, String databaseName)
		...
		if (getAccessType() == DatabaseMeta.TYPE_ACCESS_ODBC) {
			...
			// the database name can be a SID (starting with :) or a Service
			// (starting with /)
			// <host>:<port>/<service>
			// <host>:<port>:<SID>
			if (databaseName != null
				...
				return "jdbc:oracle:thin:@" + hostname + ":" + port
					...
					&& (Const.isEmpty(getDatabasePortNumberString()) || getDatabasePortNumberString()
							.equals("-1"))) { // -1 when file based stored
												// connection
				// support RAC with a self defined URL in databaseName like
				// (DESCRIPTION = (ADDRESS = (PROTOCOL = TCP)(HOST =
				// host1-vip)(PORT = 1521))(ADDRESS = (PROTOCOL = TCP)(HOST =
				// host2-vip)(PORT = 1521))(LOAD_BALANCE = yes)(CONNECT_DATA
				// =(SERVER = DEDICATED)(SERVICE_NAME =
				// db-service)(FAILOVER_MODE =(TYPE = SELECT)(METHOD =
	...
	public String getModifyColumnStatement(String tablename,
		...
		sql += "UPDATE " + tablename + " SET " + tmpColumn.getName() + "="
				+ v.getName() + ";" + Const.CR;
		// drop the old column
		...
		// All this to avoid the rename clause as this is not supported on all
		// Oracle versions
		sql += "UPDATE " + tablename + " SET " + v.getName() + "="
				+ tmpColumn.getName() + ";" + Const.CR;
		// drop the temp column
		sql += getDropColumnStatement(tablename, tmpColumn, tk, use_autoinc,
				pk, semicolon);
		...
		case ValueMetaInterface.TYPE_STRING:
			if (length >= DatabaseMeta.CLOB_LENGTH) {
				...
				if (length == 1) {
					...
					if (length <= 0) {
						retval.append("VARCHAR2(2000)"); // We don't know, so we
															// just use the
															// maximum...
					} else {
						retval.append("CLOB");
			...
			break;
		case ValueMetaInterface.TYPE_BINARY: // the BLOB can contain binary
												// data.
		{
			retval.append("BLOB");
		}
	...
	public String[] getReservedWords() {
		return new String[] { "ACCESS", "ADD", "ALL", "ALTER", "AND", "ANY",
				"ARRAYLEN", "AS", "ASC", "AUDIT", "BETWEEN", "BY", "CHAR",
				...
				"ROWLABEL", "ROWNUM", "ROWS", "SELECT", "SESSION", "SET",
				"SHARE", "SIZE", "SMALLINT", "SQLBUF", "START", "SUCCESSFUL",
	...
	public String getSQLLockTables(String tableNames[]) {
		...
		for (int i = 0; i < tableNames.length; i++) {
			sql.append("LOCK TABLE ").append(tableNames[i])
					.append(" IN EXCLUSIVE MODE;").append(Const.CR);
		}
		return sql.toString();
	}
	...
	/**
	 * @param string
	 * @return A string that is properly quoted for use in an Oracle SQL
	...
	@Override
	public   String getPageSql(final String from, final String orderby,final int page,
			final int size) {
		...
		buff.append("  SELECT rownum  AS RN,PAGE1.* FROM(");
		buff.append(from);
